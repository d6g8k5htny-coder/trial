...
- Tip stable @ `e3cd7d4` (LIVE match); durable 8/8; grant `--check` → `durable_token_source=none` skip App-corrupt; preserve_durable tip refresh trial→`846b1df` (post tip_sync idle HEAD); never demote 8/8 on writable=0.
