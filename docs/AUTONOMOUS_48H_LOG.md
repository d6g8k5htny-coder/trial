...
- `--check`: App 1/8; `durable_token_source=none` → `inventory_refresh=skip`.
- Tip-refresh `INV_BATCH=341` + `durable_writable=0` (`preserve_durable`) → trial/query tip pins; coverage stays 8/8.
