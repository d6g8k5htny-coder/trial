...
Hardening tip BASE **stable keep-prior** @ `2f7a5a9` (LIVE `012d5be` tip_moved). tip_or_eng: living script_stale republish batch241; unfreeze VERIFY/last-resort 560→561; Soft Intent single-header; inv parent-pin. Evidence: `portable/BATCH561_TIP_ENG_BRIEF.json` + `BATCH561_LIVING_REPUBLISH_BRIEF.json`. Intent soften 441/445 preserved. `lemma_closed=false`. Goal OPEN.

```bash
./scripts/refresh_path_c_bundle.sh --dry-run
./scripts/republish_living_path_c_release.sh --dry-run
python3 -m pytest tests/test_intent.py::test_batch561_tip_or_eng_tip_drift_keep_prior_unfreeze -q
```
