...
Hardening tip **stable** @ `2f7a5a9`. tip_or_eng: tip match; living script_stale republish + unfreeze 507→508; inv parent-pin. Soft Intent single === header (no re-stack). `lemma_closed=false`. Goal OPEN.

```bash
./scripts/refresh_path_c_bundle.sh --dry-run
./scripts/republish_living_path_c_release.sh --dry-run
python3 -m pytest tests/test_intent.py::test_batch508_tip_or_eng_living_script -q
```
