...
Hardening tip **stable** @ `e3cd7d4`. Assignment `grant_check_dual_vector_8of8`: `--check` → `durable_token_source=none` skip App-corrupt; preserve_durable tip refresh trial→`a4a42a1`; INV_BATCH→359; coverage 8/8_WRITABLE; `BATCH359_GRANT.json`. Goal OPEN. `lemma_closed=false`. Scientific effect: NONE.
