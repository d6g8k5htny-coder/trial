...
Default tip is **ALIGNED** @ `1c6e74b` (Batch 60; owner [PR #41](https://github.com/d6g8k5htny-coder/main/pull/41) renew after #32/`c2b0620`). **HOLD on PR #2 is VOID**. Path A OR Path B OK when misaligned; prefer **Path B**. `./scripts/restore_main_face.sh` short-circuits when already ALIGNED. Path C portable patches on hardening BASE_TIP `b3da668` (post-#34; **no 0017**; `owner_land_path_c --dry-run` → keep hardening). Trial write to `main` still **403**.
...
- [`portable/BATCH60_TOKEN_SEARCH.json`](portable/BATCH60_TOKEN_SEARCH.json) — redacted token/write probe log (Batch 59 log retained)
- [`portable/RESTORE_PLAN_60.json`](portable/RESTORE_PLAN_60.json) — live restore plan currency
