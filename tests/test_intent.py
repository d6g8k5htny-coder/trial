...
def test_batch357_inventory_preserve_durable_tip_pin() -> None:
    """Batch 357: preserve_durable tip pin after Batch 356 lands; tip e3cd7d4."""
    import json
    import re
...
def test_batch357_tip_or_eng_continue() -> None:
    """Batch 357: tip_or_eng — inv tip re-pin after conflict-marker land."""
    import json
    ...
    assert hunt.get("lemma_closed") is False
    assert hunt.get("defect_shipped") is True
    assert hunt.get("action") == "inventory_tip_repin_after_land_head"
    ...
    assert "tip_or_eng" in unblock and "Batch 357" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 357 tip-eng)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
