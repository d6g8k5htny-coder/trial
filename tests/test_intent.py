...
def test_batch355_inventory_preserve_durable_tip_pin() -> None:
    ...
    assert evidence.get("scientific_effect") == "NONE"
    assert _living_tip(str(evidence.get("hardening_tip") or ""))
    # Soft: trial tip may lag one land behind HEAD after peer merges.
    assert evidence.get("trial_tip_matches_live_head") in (True, False)
...
def test_batch355_ci_audit_watch_idle() -> None:
    ...
    art = json.loads(
        (ROOT / "portable" / "BATCH355_CI_AUDIT_WATCH.json").read_text(encoding="utf-8")
    )
    assert art.get("batch") == "355"
    assert art.get("action") == "idle_no_commit"
    assert art.get("ci_status") == "green"
    assert art.get("lemma_closed") is False
    assert art.get("flipped_anything") is False
    ...
    assert art.get("goal") == "OPEN"
    assert art.get("tip_match") is True
    assert len(art.get("ci_green_runs") or []) >= 2
    assert _living_tip(str(art.get("hardening_tip") or art.get("tip") or ""))

    brief = json.loads(
        (ROOT / "portable" / "BATCH355_CI_AUDIT_IDLE_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("action") == "idle_no_commit"
    assert brief.get("ci_status") == "green"
    assert brief.get("lemma_closed") is False
    ...
    body = intent[start:end]
    assert "Living last-resort" in body

    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
    _assert_print_owner_header_batch_at_least(unblock, 355)
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 355 ci-audit-watch)" in land
    status = json.loads(
        (ROOT / "portable" / "PATH_C_STATUS.json").read_text(encoding="utf-8")
    )
    assert status.get("lemma_closed") is False

def test_batch356_idle_tip_sync_watch() -> None:
    """Batch 356: tip stable @e3cd7d4; idle_no_commit align-watch evidence."""
    import json

    tiny = json.loads(
        (ROOT / "portable" / "BATCH356_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "356"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH356_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH356_IDLE_BRIEF.json").read_text(encoding="utf-8")
    )
    ...
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
    _assert_print_owner_header_batch_at_least(unblock, 356)
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 356 idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 356 idle)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch356_inventory_preserve_durable_tip_pin() -> None:
    ...
    trial = next(
        (
            d
            for d in (inv.get("details") or [])
            if str(d.get("name") or d.get("full_name") or d.get("repo") or "").endswith("/trial")
            or (
                isinstance(d, dict)
                and (d.get("tip_sha") or "").startswith(
                    str(inv.get("details", [{}])[-1].get("tip_sha", "")[:7])
                )
            )
        ),
        inv["details"][7],
    ...
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("3eba995") and not tip.startswith("b6eb658")
    ...
    hunt = json.loads(
        (ROOT / "portable" / "BATCH356_INV_TIP_PIN_HUNT.json").read_text(encoding="utf-8")
    )
    assert hunt.get("defect_shipped") is True
    assert hunt.get("defect_id") == "inventory_trial_tip_lag_after_lands"
...
def test_batch356_tip_or_eng_continue() -> None:
    ...
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("b6eb658")

    brief = json.loads(
        (ROOT / "portable" / "BATCH356_TIP_ENG_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("batch") == "356"
    assert brief.get("lemma_closed") is False
    assert brief.get("flipped_anything") is False
    assert brief.get("action") in ("inventory_tip_repin_after_land_head", "eng_conflict_marker_fix_and_inv_tip_repin")
    assert brief.get("trial_tip_matches_live_head") is True
    assert brief.get("goal") == "OPEN"
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))

    hunt = json.loads(
        (ROOT / "portable" / "BATCH356_TIP_ENG_HUNT.json").read_text(encoding="utf-8")
    )
    assert hunt.get("lemma_closed") is False
    assert hunt.get("defect_shipped") is True

    evidence = json.loads(
        (ROOT / "portable" / "BATCH356_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") in ("inventory_tip_repin_after_land_head", "eng_conflict_marker_fix_and_inv_tip_repin")
    assert evidence.get("trial_tip_matches_live_head") is True
    assert _living_tip(str(evidence.get("hardening_tip") or ""))
    ...
    assert "tip_or_eng" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 356 tip-eng)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 356 tip-eng)" in owner
...
def test_batch357_idle_tip_sync_watch() -> None:
    """Batch 357: tip_sync_watch idle @e3cd7d4; tip_match; living current."""
    import json

    tiny = json.loads(
        (ROOT / "portable" / "BATCH357_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "357"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH357_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH357_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("assignment") == "tip_sync_watch_vs_BASE_TIP_e3cd7d4"
    assert brief.get("action") == "idle_no_commit"
    assert brief.get("lemma_closed") is False
    ...
    _assert_print_owner_header_batch_at_least(unblock, 357)
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 357 idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
...
def test_batch357_inventory_preserve_durable_tip_pin() -> None:
    ...
    assert int(str(inv.get("batch") or "0")) >= 357
    assert inv.get("durable_writable") == "8/8"
    assert inv.get("durable_sibling_coverage") == "8/8_WRITABLE"
    assert inv.get("lemma_closed") is False
    assert inv.get("flipped_anything") is False
    ...
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("a7f4fce") and not tip.startswith("b6eb658")

    evidence = json.loads(
        (ROOT / "portable" / "BATCH357_INV_TIP_PIN_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("batch") == "357"
    assert evidence.get("action") == "inventory_preserve_durable_tip_pin"
    assert evidence.get("durable") == "8/8_WRITABLE"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH357_INV_TIP_PIN_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("batch") == "357"
    assert brief.get("action") == "inventory_preserve_durable_tip_pin"
    assert brief.get("lemma_closed") is False
    ...
    hunt = json.loads(
        (ROOT / "portable" / "BATCH357_INV_TIP_PIN_HUNT.json").read_text(encoding="utf-8")
    )
    assert hunt.get("defect_shipped") is True

    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
    headers = re.findall(r"=== Batch (\d+)\s", unblock)
    assert headers and int(headers[0]) >= 357 and len(headers) == 1
    assert "Batch 357" in unblock and "tip_or_eng" in unblock
...
def test_batch357_tip_or_eng_continue() -> None:
    ...
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("a7f4fce")

    brief = json.loads(
        (ROOT / "portable" / "BATCH357_TIP_ENG_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("batch") == "357"
    assert brief.get("lemma_closed") is False
    assert brief.get("flipped_anything") is False
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH357_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
    assert evidence.get("action") in (
...
def test_batch358_inventory_preserve_durable_tip_pin() -> None:
    ...
    assert int(str(inv.get("batch") or "0")) >= 358
    assert inv.get("durable_writable") == "8/8"
    assert inv.get("durable_sibling_coverage") == "8/8_WRITABLE"
    assert inv.get("lemma_closed") is False
    assert inv.get("flipped_anything") is False
    ...
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("8431393") and not tip.startswith("a7f4fce")

    evidence = json.loads(
        (ROOT / "portable" / "BATCH358_INV_TIP_PIN_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("batch") == "358"
    assert evidence.get("action") == "inventory_preserve_durable_tip_pin"
    assert evidence.get("durable") == "8/8_WRITABLE"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH358_INV_TIP_PIN_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("batch") == "358"
    assert brief.get("action") == "inventory_preserve_durable_tip_pin"
    assert brief.get("lemma_closed") is False
    ...
    hunt = json.loads(
        (ROOT / "portable" / "BATCH358_INV_TIP_PIN_HUNT.json").read_text(encoding="utf-8")
    )
    assert hunt.get("defect_shipped") is True
    ...
    assert "Batch 358" in unblock and "tip_or_eng" in unblock
...
def test_batch358_tip_or_eng_continue() -> None:
    ...
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("8431393")

    brief = json.loads(
        (ROOT / "portable" / "BATCH358_TIP_ENG_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("batch") == "358"
    assert brief.get("lemma_closed") is False
    assert brief.get("flipped_anything") is False
    ...
    hunt = json.loads(
        (ROOT / "portable" / "BATCH358_TIP_ENG_HUNT.json").read_text(encoding="utf-8")
    )
    assert hunt.get("lemma_closed") is False
    assert hunt.get("defect_shipped") is True

    evidence = json.loads(
        (ROOT / "portable" / "BATCH358_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
...
def test_batch359_idle_tip_sync_watch() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH359_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "359"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH359_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH359_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("assignment") == "tip_sync_watch_vs_BASE_TIP_e3cd7d4"
    assert brief.get("action") == "idle_no_commit"
    assert brief.get("lemma_closed") is False
    ...
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 359 idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
...
def test_batch359_grant_inventory_refresh() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH359_GRANT.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "359"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("coverage") == "8/8_WRITABLE"
    assert tiny.get("durable") == "8/8"
    assert tiny.get("action") == "grant_inventory_refresh_batch359"
    assert tiny.get("assignment") == "grant_check_dual_vector_8of8"
    assert tiny.get("tip_match") is True
    assert tiny.get("inventable_promoted") is False
    assert tiny.get("goal") == "OPEN"
    ...
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 359 grant)" in land
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch359_tip_or_eng_continue() -> None:
    ...
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("77f6b8c") and not tip.startswith("a4a42a1")

    brief = json.loads(
        (ROOT / "portable" / "BATCH359_TIP_ENG_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("batch") == "359"
    assert brief.get("lemma_closed") is False
    assert brief.get("flipped_anything") is False
    ...
    hunt = json.loads(
        (ROOT / "portable" / "BATCH359_TIP_ENG_HUNT.json").read_text(encoding="utf-8")
    )
    assert hunt.get("lemma_closed") is False
    assert hunt.get("defect_shipped") is True

    evidence = json.loads(
        (ROOT / "portable" / "BATCH359_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
...
def test_batch359_multi_agent_wake_assign() -> None:
    ...
    wake = json.loads(
        (ROOT / "portable" / "MULTI_AGENT_WAKE_BATCH359.json").read_text(encoding="utf-8")
    )
    assert wake.get("batch") == 359
    assert wake.get("wake359_on_main") is True
    assert wake.get("lemma_closed") is False
    assert wake.get("flipped_anything") is False
    assert wake.get("action") == "multi_agent_wake_and_assign"
    assert wake.get("scientific_effect") == "NONE"
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH359_WAKE_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("batch") == "359"
    assert brief.get("action") == "multi_agent_wake_and_assign"
    assert brief.get("lemma_closed") is False
    ...
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
    _assert_print_owner_header_batch_at_least(unblock, 359)
    assert "WAKE359" in unblock or "MULTI_AGENT wake" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 359 wake)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "MULTI_AGENT_WAKE_BATCH359" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
    assert "stopped agents" in log_md and "Batch 359" in log_md
    ...
    m = re.search(r'(?m)^    return "(\d+)"\s*$', poster)
    assert m is not None
    assert int(m.group(1)) >= 359
    helper = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(
        encoding="utf-8"
    )
    m_inv = re.search(r'return "(\d+)"', helper)
    assert m_inv is not None
    assert int(m_inv.group(1)) >= 359
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 359)
...
def test_batch359_living_script_stale_republish_after_wake() -> None:
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH359_WAKE_REPUBLISH_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert brief.get("batch") == "359"
    ...
    assert brief.get("flipped_anything") is False
    assert brief.get("scientific_effect") == "NONE"
    assert brief.get("defect_id") == "living_script_stale_after_batch359_wake"
    assert brief.get("action") == "eng_living_script_stale_republish"
    assert brief.get("tip_match") is True
    assert brief.get("inventable_promoted") is False
    assert brief.get("goal_complete") is False
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    living = brief.get("living") or {}
    assert living.get("tip_stale") == 0
    assert living.get("script_stale") == 0

    hunt = json.loads(
        (ROOT / "portable" / "BATCH359_WAKE_REPUBLISH_HUNT.json").read_text(
            encoding="utf-8"
        )
    )
    assert hunt.get("defect_id") == "living_script_stale_after_batch359_wake"
    assert hunt.get("lemma_closed") is False
    assert hunt.get("tip_moved") is False
    assert hunt.get("defect_shipped") is True

    ev = json.loads(
        (ROOT / "portable" / "BATCH359_WAKE_REPUBLISH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert ev.get("script_stale_before") == 1
    assert ev.get("script_stale_after") == 0
    assert ev.get("lemma_closed") is False
    assert ev.get("action") == "eng_living_script_stale_republish"

    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
    _assert_print_owner_header_batch_at_least(unblock, 359)
    assert "script_stale" in unblock and "WAKE359" in unblock

    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 359 wake-republish)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
...
def test_batch360_idle_tip_sync_watch() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH360_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "360"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH360_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH360_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("assignment") == "tip_sync_watch_vs_BASE_TIP_e3cd7d4"
    assert brief.get("action") == "idle_no_commit"
    assert brief.get("lemma_closed") is False
    ...
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 360 idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 360 idle)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
    assert "Batch 360" in log_md and "idle_no_commit" in log_md

def test_batch360_tip_or_eng_continue() -> None:
    """Batch 360: tip_or_eng — inv tip re-pin + last-resort unfreeze + living republish."""
    import json
    inv = json.loads((ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8"))
    assert int(str(inv.get("batch") or "0")) >= 360
    assert inv.get("lemma_closed") is False
    trial = next(d for d in (inv.get("details") or []) if str(d.get("name") or "").endswith("/trial"))
    assert str(trial.get("tip_sha") or "") and not str(trial.get("tip_sha")).startswith("4146b8e")
    brief = json.loads((ROOT / "portable" / "BATCH360_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "360" and brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_inv_tip_repin_and_living_republish"
    assert brief.get("trial_tip_matches_live_head") is True and brief.get("goal") == "OPEN"
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH360_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
...
def test_batch361_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH361_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "361"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    assert living.get("tip_stale") == 0
    assert living.get("script_stale") == 0
    delta = watch.get("delta_vs_batch359") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0

    assert not (ROOT / "portable" / "BATCH361_RESEARCH_STACK_AUDIT.json").exists()
    prior = json.loads(
        (ROOT / "portable" / "BATCH357_RESEARCH_STACK_AUDIT.json").read_text(
            encoding="utf-8"
        )
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH361_RESEARCH_STACK_AUDIT_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert brief.get("batch") == "361"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "research_stack_audit_watch"
    assert brief.get("audit_delta_needed") is False

    evidence = json.loads(
        (ROOT / "portable" / "BATCH361_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "361"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("goal") == "OPEN"
    assert _living_tip(str(evidence.get("hardening_tip") or ""))
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 361 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 361 research-audit-watch)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
    assert "Batch 361" in log_md and "research_stack_audit_watch" in log_md
def test_batch361_idle_tip_sync_watch() -> None:
    """Batch 361: tip_sync_watch idle @e3cd7d4; tip_match; living current."""
    import json

    tiny = json.loads(
        (ROOT / "portable" / "BATCH361_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "361"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH361_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH361_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("assignment") == "tip_sync_watch_vs_BASE_TIP_e3cd7d4"
    assert brief.get("action") == "idle_no_commit"
    assert brief.get("lemma_closed") is False
    ...
    _assert_print_owner_header_batch_at_least(unblock, 361)
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 361 idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 361 idle)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
    assert "Batch 361" in log_md and "idle_no_commit" in log_md

def test_batch361_tip_or_eng_continue() -> None:
    """Batch 361: tip_or_eng — inv tip re-pin beyond parent + living republish."""
    import json
    inv = json.loads((ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8"))
    assert int(str(inv.get("batch") or "0")) >= 361
    assert inv.get("lemma_closed") is False
    trial = next(d for d in (inv.get("details") or []) if str(d.get("name") or "").endswith("/trial"))
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("6108a30") and not tip.startswith("b8b7f8e")
    brief = json.loads((ROOT / "portable" / "BATCH361_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "361" and brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_inv_tip_repin_and_living_republish"
    assert brief.get("trial_tip_matches_live_head") is True and brief.get("goal") == "OPEN"
    assert brief.get("lag_beyond_parent") is True
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH361_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
...
def test_batch362_idle_tip_sync_watch() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH362_IDLE.json").read_text(encoding="utf-8")
    )
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH362_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH362_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("assignment") == "tip_sync_watch_vs_BASE_TIP_e3cd7d4"
    assert brief.get("action") == "idle_no_commit"
    assert brief.get("lemma_closed") is False
...
def test_batch362_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH362_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "362"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    assert living.get("tip_stale") == 0
    assert living.get("script_stale") == 0
    delta = watch.get("delta_vs_batch361") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0

    assert not (ROOT / "portable" / "BATCH362_RESEARCH_STACK_AUDIT.json").exists()
    prior = json.loads(
        (ROOT / "portable" / "BATCH357_RESEARCH_STACK_AUDIT.json").read_text(
            encoding="utf-8"
        )
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH362_RESEARCH_STACK_AUDIT_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH362_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "362"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("audit_delta_needed") is False
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 362 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
...
def test_batch363_tip_or_eng_continue() -> None:
    ...
    import json
    brief = json.loads((ROOT / "portable" / "BATCH363_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "363"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_living_script_stale_republish"
    assert brief.get("tip_match") is True
    assert brief.get("goal") == "OPEN"
    assert brief.get("defect_id") == "living_verify_refresh_batch_lag_345_vs_362"
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH363_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH363_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_living_script_stale_republish"
    assert evidence.get("tip_match") is True
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(str(verify.get("refresh_batch") or "0")) >= 362
    assert verify.get("lemma_closed") is False
    tiny = json.loads((ROOT / "portable" / "BATCH363_IDLE.json").read_text(encoding="utf-8"))
    assert tiny.get("action") == "eng_living_script_stale_republish"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("tip_match") is True
    living = tiny.get("living") or {}
    assert living.get("script_stale") == 0
...
def test_batch363_inventory_preserve_durable_tip_pin() -> None:
    ...
    evidence = json.loads((ROOT / "portable" / "BATCH363_INV_TIP_PIN_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("batch") == "363"
    assert evidence.get("action") == "inventory_preserve_durable_tip_pin"
    assert evidence.get("trial_tip_matches_live_head") is True
    assert evidence.get("defect_id") == "inventory_trial_tip_lag_after_lands"
    assert _living_tip(str(evidence.get("hardening_tip") or ""))
    brief = json.loads((ROOT / "portable" / "BATCH363_INV_TIP_PIN_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("action") == "inventory_preserve_durable_tip_pin"
    hunt = json.loads((ROOT / "portable" / "BATCH363_INV_TIP_PIN_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    post = json.loads((ROOT / "portable" / "BATCH363_POST_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert post.get("action") == "inventory_tip_repin_after_land_head"
    assert post.get("lag_beyond_parent") is True
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 363)
    helper = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', helper)]
    assert _inv_rets and max(_inv_rets) >= 363
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
    headers = re.findall(r"=== Batch (\d+)\s", unblock)
...
def test_batch364_idle_tip_sync_watch() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH364_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "364"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH364_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH364_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("assignment") == "tip_sync_watch_vs_BASE_TIP_e3cd7d4"
    assert brief.get("action") == "idle_no_commit"
    assert brief.get("lemma_closed") is False
    ...
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 364 idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 364 idle)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch364_tip_or_eng_continue() -> None:
    ...
    import re
    inv = json.loads((ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8"))
    assert int(str(inv.get("batch") or "0")) >= 364
    assert inv.get("lemma_closed") is False
    assert inv.get("durable_sibling_coverage") == "8/8_WRITABLE"
    trial = next(d for d in (inv.get("details") or []) if str(d.get("name") or "").endswith("/trial"))
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("108c91a")
    brief = json.loads((ROOT / "portable" / "BATCH364_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "364" and brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_inv_tip_repin_and_living_republish"
    assert brief.get("trial_tip_matches_live_head") is True
    assert brief.get("lag_beyond_parent") is True
    assert brief.get("goal") == "OPEN"
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH364_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH364_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_inv_tip_repin_and_living_republish"
    assert evidence.get("tip_match") is True
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 364)
    helper = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in re.findall(r'return "(\d+)"', helper)]
    assert _inv_rets and max(_inv_rets) >= 364
...
def test_batch365_tip_or_eng_continue() -> None:
    ...
    import json
    brief = json.loads((ROOT / "portable" / "BATCH365_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "365"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_living_script_stale_republish"
    assert brief.get("tip_match") is True
    assert brief.get("goal") == "OPEN"
    assert brief.get("defect_id") == "living_verify_refresh_batch_lag_363_vs_364"
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH365_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH365_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_living_script_stale_republish"
    assert evidence.get("tip_match") is True
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(str(verify.get("refresh_batch") or "0")) >= 364
    assert verify.get("lemma_closed") is False
    tiny = json.loads((ROOT / "portable" / "BATCH365_IDLE.json").read_text(encoding="utf-8"))
    assert tiny.get("action") == "eng_living_script_stale_republish"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("tip_match") is True
    living = tiny.get("living") or {}
...
def test_batch365_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH365_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "365"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    assert living.get("tip_stale") == 0
    assert living.get("script_stale") == 0
    delta = watch.get("delta_vs_batch362") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0

    assert not (ROOT / "portable" / "BATCH365_RESEARCH_STACK_AUDIT.json").exists()
    prior = json.loads(
        (ROOT / "portable" / "BATCH357_RESEARCH_STACK_AUDIT.json").read_text(
            encoding="utf-8"
        )
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH365_RESEARCH_STACK_AUDIT_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert brief.get("batch") == "365"
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH365_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "365"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("audit_delta_needed") is False
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 365 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 365 research-audit-watch)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch365_unfreeze_verify_refresh_batch() -> None:
    ...
    import json
    import re
    brief = json.loads((ROOT / "portable" / "BATCH365_VERIFY_LAG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "365"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_living_verify_refresh_batch_bump"
    assert brief.get("defect_id") == "living_verify_refresh_batch_lag_364_vs_365"
    assert brief.get("goal") == "OPEN"
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH365_VERIFY_LAG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH365_VERIFY_LAG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_living_verify_refresh_batch_bump"
    assert str(evidence.get("verify_refresh_batch") or "") == "365"
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(str(verify.get("refresh_batch") or "0")) >= 365
    assert verify.get("lemma_closed") is False
    unfreeze = json.loads((ROOT / "portable" / "BATCH365_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
    assert unfreeze.get("action") == "eng_unfreeze_last_resort_364_to_365"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 365)
    helper = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in re.findall(r'return "(\d+)"', helper)]
    assert _inv_rets and max(_inv_rets) >= 365
    poster = (ROOT / "scripts" / "post_batch322_wake_comments.py").read_text(encoding="utf-8")
...
def test_batch366_idle_tip_sync_watch() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH366_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "366"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH366_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH366_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("assignment") == "tip_sync_watch_vs_BASE_TIP_e3cd7d4"
    assert brief.get("action") == "idle_no_commit"
    assert brief.get("lemma_closed") is False
    ...
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 366 idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 366 idle)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch366_tip_or_eng_continue() -> None:
    ...
    import re
    inv = json.loads((ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8"))
    assert int(str(inv.get("batch") or "0")) >= 366
    assert inv.get("lemma_closed") is False
    assert inv.get("durable_sibling_coverage") == "8/8_WRITABLE"
    trial = next(d for d in (inv.get("details") or []) if str(d.get("name") or "").endswith("/trial"))
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("938daf0")
    brief = json.loads((ROOT / "portable" / "BATCH366_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "366" and brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_inv_tip_repin_and_living_republish"
    assert brief.get("trial_tip_matches_live_head") is True
    assert brief.get("lag_beyond_parent") is True
    assert brief.get("goal") == "OPEN"
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH366_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH366_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_inv_tip_repin_and_living_republish"
    assert evidence.get("tip_match") is True
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(str(verify.get("refresh_batch") or "0")) >= 366
    assert verify.get("lemma_closed") is False
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 366)
    helper = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in re.findall(r'return "(\d+)"', helper)]
    assert _inv_rets and max(_inv_rets) >= 366
    poster = (ROOT / "scripts" / "post_batch322_wake_comments.py").read_text(encoding="utf-8")
    _wake_rets = [int(x) for x in re.findall(r'return "(\d+)"', poster.split("def batch_marker")[0])]
...
def test_batch367_idle_tip_sync_watch() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH367_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "367"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH367_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH367_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("assignment") == "tip_sync_watch_vs_BASE_TIP_e3cd7d4"
    assert brief.get("action") == "idle_no_commit"
    assert brief.get("lemma_closed") is False
    ...
    _assert_print_owner_header_batch_at_least(unblock, 367)
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 367 idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
...
def test_batch368_idle_tip_sync_watch() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH368_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "368"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH368_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
...
def test_batch368_tip_or_eng_continue() -> None:
    ...
    import re
    inv = json.loads((ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8"))
    assert int(str(inv.get("batch") or "0")) >= 368
    assert inv.get("lemma_closed") is False
    assert inv.get("durable_sibling_coverage") == "8/8_WRITABLE"
    trial = next(d for d in (inv.get("details") or []) if str(d.get("name") or "").endswith("/trial"))
    tip = str(trial.get("tip_sha") or "")
    assert tip and not tip.startswith("695b421")
    brief = json.loads((ROOT / "portable" / "BATCH368_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "368" and brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_inv_tip_repin_and_living_republish"
    assert brief.get("trial_tip_matches_live_head") is True
    assert brief.get("lag_beyond_parent") is True
    assert brief.get("goal") == "OPEN"
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH368_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH368_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_inv_tip_repin_and_living_republish"
    assert evidence.get("tip_match") is True
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(str(verify.get("refresh_batch") or "0")) >= 368
    assert verify.get("lemma_closed") is False
    # Living >=368 helpers only (no REFRESH any() allowlists).
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 368)
    helper = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in re.findall(r'return "(\d+)"', helper)]
    assert _inv_rets and max(_inv_rets) >= 368
    poster = (ROOT / "scripts" / "post_batch322_wake_comments.py").read_text(encoding="utf-8")
    _wake_rets = [int(x) for x in re.findall(r'return "(\d+)"', poster.split("def batch_marker")[0])]
...
def test_batch369_tip_or_eng_wake_inv_living() -> None:
    ...
    import re
    inv = json.loads((ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8"))
    assert int(str(inv.get("batch") or "0")) >= 369
    assert inv.get("lemma_closed") is False
    assert inv.get("durable_sibling_coverage") == "8/8_WRITABLE"
    trial = next(d for d in (inv.get("details") or []) if str(d.get("name") or "").endswith("/trial"))
    tip = str(trial.get("tip_sha") or "")
    ...
    assert brief.get("goal") == "OPEN"
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH369_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    wake_brief = json.loads((ROOT / "portable" / "BATCH369_WAKE_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
...
def test_batch369_idle_tip_sync_watch() -> None:
    ...
    unfreeze = json.loads(
        (ROOT / "portable" / "BATCH369_UNFREEZE_BRIEF.json").read_text(encoding="utf-8")
    )
    assert unfreeze.get("to_batch") == "369"
    ...
    assert int(verify.get("refresh_batch") or 0) >= 369
    assert verify.get("lemma_closed") is False
...
def test_batch369_living_script_stale_republish() -> None:
    ...
    import json
    brief = json.loads((ROOT / "portable" / "BATCH369_LIVING_REPUBLISH_BRIEF.json").read_text(encoding="utf-8"))
    ...
    assert brief.get("goal") == "OPEN"
    assert brief.get("script_stale_after") == 0
    assert brief.get("script_stale_before") == 1
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH369_LIVING_REPUBLISH_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH369_LIVING_REPUBLISH_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_living_script_stale_republish"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 369)
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
...
def test_batch369_tip_or_eng_continue() -> None:
    ...
    hunt = json.loads((ROOT / "portable" / "BATCH369_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH369_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_inv_tip_repin_and_living_republish"
    assert evidence.get("tip_match") is True
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 369)
...
def test_batch369_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH369_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "369"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    living = watch.get("living") or {}
    assert living.get("tip_stale") == 0
    assert int(living.get("script_stale") or 0) in (0, 1)
    delta = watch.get("delta_vs_batch367") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0

    assert not (ROOT / "portable" / "BATCH369_RESEARCH_STACK_AUDIT.json").exists()

    brief = json.loads(
        (ROOT / "portable" / "BATCH369_RESEARCH_STACK_AUDIT_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert brief.get("batch") == "369"
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH369_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "369"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("audit_delta_needed") is False
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 369 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
...
def test_batch370_inv_tip_pin_after_eng_hunt() -> None:
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH370_INV_TIP_PIN_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("batch") == "370"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "inventory_preserve_durable_tip_pin"
    ...
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads(
        (ROOT / "portable" / "BATCH370_INV_TIP_PIN_HUNT.json").read_text(encoding="utf-8")
    )
    assert hunt.get("defect_shipped") is True
    inv = json.loads(
        (ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8")
...
def test_batch371_idle_eng_hunt() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH371_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "371"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH371_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("tip_match") is True
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH371_BRIEF.json").read_text(encoding="utf-8")
    )
    ...
    hunt = json.loads(
        (ROOT / "portable" / "BATCH371_IDLE_HUNT.json").read_text(encoding="utf-8")
    )
    assert hunt.get("defect_shipped") is False
    assert hunt.get("action") == "idle_no_commit"
    ...
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 371 idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 371 idle)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch371_post_soften_inv_tip_pin() -> None:
    ...
    import json
    brief = json.loads((ROOT / "portable" / "BATCH371_POST_SOFTEN_INV_TIP_PIN_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "371"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "inventory_preserve_durable_tip_pin"
    assert brief.get("tip_match") is True
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH371_POST_SOFTEN_INV_TIP_PIN_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    inv = json.loads((ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8"))
    assert int(str(inv.get("batch") or "0")) >= 371
    assert inv.get("durable_sibling_coverage") == "8/8_WRITABLE"
...
def test_batch372_research_stack_audit_watch() -> None:
    ...
    import json
    watch = json.loads((ROOT / "portable" / "BATCH372_RESEARCH_AUDIT_WATCH.json").read_text(encoding="utf-8"))
    assert watch.get("batch") == "372"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    delta = watch.get("delta_vs_batch369") or {}
    assert delta.get("open_premises") == 0
    assert delta.get("open_lemmas") == 0
    assert delta.get("open_prizes") == 0
    assert _living_tip(str(watch.get("hardening_tip") or ""))
    brief = json.loads((ROOT / "portable" / "BATCH372_RESEARCH_STACK_AUDIT_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("lemma_closed") is False
    unfreeze = json.loads((ROOT / "portable" / "BATCH372_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
    assert unfreeze.get("to_batch") == "372"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    ...
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(verify.get("refresh_batch") or 0) >= 372
    assert verify.get("lemma_closed") is False
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
...
def test_batch373_inv_tip_pin_unfreeze() -> None:
    ...
    import json
    brief = json.loads((ROOT / "portable" / "BATCH373_INV_TIP_PIN_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "373"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "inventory_preserve_durable_tip_pin"
    assert brief.get("tip_match") is True
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH373_INV_TIP_PIN_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    idle = json.loads((ROOT / "portable" / "BATCH373_IDLE.json").read_text(encoding="utf-8"))
    assert idle.get("action") == "idle_no_commit"
    assert idle.get("lemma_closed") is False
    unfreeze = json.loads((ROOT / "portable" / "BATCH373_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
    assert unfreeze.get("to_batch") == "373"
    inv = json.loads((ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8"))
    assert int(str(inv.get("batch") or "0")) >= 373
    assert inv.get("durable_sibling_coverage") == "8/8_WRITABLE"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
...
def test_batch373_eng_hunt_idle() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH373_ENG_HUNT_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "373"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("action") == "idle_no_commit"
    ...
    assert living.get("script_stale") == 0
    brief = json.loads(
        (ROOT / "portable" / "BATCH373_ENG_HUNT_BRIEF.json").read_text(encoding="utf-8")
    )
    assert brief.get("assignment") == "eng_defect_hunt_no_claim_promotion"
    assert brief.get("action") == "idle_no_commit"
    hunt = json.loads(
        (ROOT / "portable" / "BATCH373_ENG_HUNT_HUNT.json").read_text(encoding="utf-8")
    )
    assert hunt.get("defect_shipped") is False
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
    _assert_print_owner_header_batch_at_least(unblock, 373)
...
def test_batch373_post_living_inv_tip_pin() -> None:
    ...
    import json
    brief = json.loads((ROOT / "portable" / "BATCH373_POST_LIVING_INV_TIP_PIN_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "373"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "inventory_preserve_durable_tip_pin"
    assert brief.get("tip_match") is True
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH373_POST_LIVING_INV_TIP_PIN_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    inv = json.loads((ROOT / "portable" / "AI_AGENT_ACCESS_INVENTORY.json").read_text(encoding="utf-8"))
    assert int(str(inv.get("batch") or "0")) >= 373
    assert inv.get("durable_sibling_coverage") == "8/8_WRITABLE"
...
def test_batch374_tip_or_eng_idle() -> None:
    ...
    import json
    import re
    tiny = json.loads((ROOT / "portable" / "BATCH374_IDLE.json").read_text(encoding="utf-8"))
    assert tiny.get("batch") == "374"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    assert living.get("tip_stale") == 0
    assert living.get("script_stale") == 0
    evidence = json.loads((ROOT / "portable" / "BATCH374_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "idle_no_commit"
    brief = json.loads((ROOT / "portable" / "BATCH374_BRIEF.json").read_text(encoding="utf-8"))
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_or_eng_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    hunt = json.loads((ROOT / "portable" / "BATCH374_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is False
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 374)
    inv_py = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
...
def test_batch374_idle_tip_sync_watch() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH374_TIP_SYNC_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "374"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH374_TIP_SYNC_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH374_TIP_SYNC_BRIEF.json").read_text(encoding="utf-8")
    )
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_sync_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    ...
    pin = json.loads(
        (ROOT / "portable" / "BATCH374_TIP_SYNC_INV_TIP_PIN_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    _assert_print_owner_header_batch_at_least(unblock, 374)
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 374 tip-sync-idle)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 374 tip-sync-idle)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch375_idle_tip_sync_watch() -> None:
    ...
    import json
    idle = json.loads((ROOT / "portable" / "BATCH375_IDLE.json").read_text(encoding="utf-8"))
    assert idle.get("batch") == "375"
    assert idle.get("lemma_closed") is False
    ...
    living = json.loads((ROOT / "portable" / "BATCH375_LIVING_REPUBLISH_BRIEF.json").read_text(encoding="utf-8"))
    assert living.get("action") == "living_tgz_content_delta_republish"
    assert living.get("living_tag") == "batch241-path-c-bundle"
    assert living.get("lemma_closed") is False
    unfreeze = json.loads((ROOT / "portable" / "BATCH375_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
    assert unfreeze.get("to_batch") == "375"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 375)
    inv_py = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', inv_py)]
    assert _inv_rets and max(_inv_rets) >= 375
    wake = (ROOT / "scripts" / "post_batch322_wake_comments.py").read_text(encoding="utf-8")
    _wake_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', wake)]
    assert _wake_rets and max(_wake_rets) >= 375
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(verify.get("refresh_batch") or 0) >= 375
    assert verify.get("lemma_closed") is False
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
...
def test_batch375_tip_or_eng_idle() -> None:
    ...
    import json
    import re
    tiny = json.loads((ROOT / "portable" / "BATCH375_TIP_ENG_IDLE.json").read_text(encoding="utf-8"))
    assert tiny.get("batch") == "375"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("action") == "idle_no_commit"
    ...
    brief = json.loads((ROOT / "portable" / "BATCH375_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("action") == "idle_no_commit"
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_or_eng_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    hunt = json.loads((ROOT / "portable" / "BATCH375_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is False
    evidence = json.loads((ROOT / "portable" / "BATCH375_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "idle_no_commit"
...
def test_batch375_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH375_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "375"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    assert int(watch.get("open_prizes") or 0) >= 3
    assert _living_tip(str(watch.get("hardening_tip") or ""))
    delta = watch.get("delta_vs_batch374") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0
    assert not (ROOT / "portable" / "BATCH375_RESEARCH_STACK_AUDIT.json").exists()

    evidence = json.loads(
        (ROOT / "portable" / "BATCH375_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "375"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("goal") == "OPEN"
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 375 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 375 research-audit-watch)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch375_tip_sync_watch_idle_parent_pin() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH375_TIP_SYNC_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "375"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH375_TIP_SYNC_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert _living_tip(str(evidence.get("hardening_tip") or ""))
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH375_TIP_SYNC_BRIEF.json").read_text(encoding="utf-8")
    )
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_sync_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    ...
    pin = json.loads(
        (ROOT / "portable" / "BATCH375_TIP_SYNC_INV_TIP_PIN_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert pin.get("action") == "inventory_preserve_durable_tip_pin"
...
def test_batch376_tip_sync_watch_idle_parent_pin() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH376_TIP_SYNC_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "376"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH376_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert _living_tip(str(evidence.get("hardening_tip") or ""))
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH376_BRIEF.json").read_text(encoding="utf-8")
    )
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_sync_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    ...
    pin = json.loads(
        (ROOT / "portable" / "BATCH376_TIP_SYNC_INV_TIP_PIN_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert pin.get("action") == "inventory_preserve_durable_tip_pin"
...
def test_batch376_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH376_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "376"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    assert int(watch.get("open_prizes") or 0) >= 3
    assert _living_tip(str(watch.get("hardening_tip") or ""))
    delta = watch.get("delta_vs_batch375") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0
    assert not (ROOT / "portable" / "BATCH376_RESEARCH_STACK_AUDIT.json").exists()

    evidence = json.loads(
        (ROOT / "portable" / "BATCH376_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "376"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("goal") == "OPEN"
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 376 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 376 research-audit-watch)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch376_tip_or_eng_living_tgz_republish() -> None:
    ...
    import json
    import re
    brief = json.loads((ROOT / "portable" / "BATCH376_TIP_ENG_LIVING_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "376"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_living_tgz_content_delta_republish"
    assert brief.get("tip_match") is True
    assert brief.get("goal") == "OPEN"
    assert brief.get("need_upload_after") == 0
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH376_TIP_ENG_LIVING_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH376_TIP_ENG_LIVING_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_living_tgz_content_delta_republish"
    assert evidence.get("need_upload_after") == 0
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 376)
    helper = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
...
def test_batch377_idle_tip_sync_watch() -> None:
    ...
    import json
    idle = json.loads((ROOT / "portable" / "BATCH377_IDLE.json").read_text(encoding="utf-8"))
    assert idle.get("batch") == "377"
    assert idle.get("lemma_closed") is False
    assert idle.get("flipped_anything") is False
    assert idle.get("tip_match") is True
    assert idle.get("action") == "idle_no_commit"
    assert _living_tip(str(idle.get("hardening_tip") or ""))
    unfreeze = json.loads((ROOT / "portable" / "BATCH377_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
    assert unfreeze.get("to_batch") == "377"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 377)
    inv_py = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', inv_py)]
    assert _inv_rets and max(_inv_rets) >= 377
    wake = (ROOT / "scripts" / "post_batch322_wake_comments.py").read_text(encoding="utf-8")
    _wake_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', wake)]
    assert _wake_rets and max(_wake_rets) >= 377
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(verify.get("refresh_batch") or 0) >= 377
    assert verify.get("lemma_closed") is False
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
...
def test_batch377_tip_sync_watch_idle_parent_pin() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH377_TIP_SYNC_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "377"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH377_TIP_SYNC_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False
    assert _living_tip(str(evidence.get("hardening_tip") or ""))
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH377_TIP_SYNC_BRIEF.json").read_text(encoding="utf-8")
    )
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_sync_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    ...
    pin = json.loads(
        (ROOT / "portable" / "BATCH377_TIP_SYNC_INV_TIP_PIN_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert pin.get("action") == "inventory_preserve_durable_tip_pin"
...
def test_batch377_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH377_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "377"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    assert int(watch.get("open_prizes") or 0) >= 3
    assert _living_tip(str(watch.get("hardening_tip") or ""))
    delta = watch.get("delta_vs_batch376") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0
    assert not (ROOT / "portable" / "BATCH377_RESEARCH_STACK_AUDIT.json").exists()

    evidence = json.loads(
        (ROOT / "portable" / "BATCH377_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "377"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("goal") == "OPEN"
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 377 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 377 research-audit-watch)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch377_tip_or_eng_living_tgz_republish() -> None:
    ...
    import json
    import re
    brief = json.loads((ROOT / "portable" / "BATCH377_TIP_ENG_LIVING_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("batch") == "377"
    assert brief.get("lemma_closed") is False
    assert brief.get("action") == "eng_living_tgz_content_delta_republish"
    assert brief.get("tip_match") is True
    assert brief.get("goal") == "OPEN"
    assert brief.get("need_upload_after") == 0
    assert _living_tip(str(brief.get("tip") or brief.get("hardening_tip") or ""))
    hunt = json.loads((ROOT / "portable" / "BATCH377_TIP_ENG_LIVING_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    evidence = json.loads((ROOT / "portable" / "BATCH377_TIP_ENG_LIVING_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "eng_living_tgz_content_delta_republish"
    assert evidence.get("need_upload_after") == 0
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 377)
...
def test_batch378_idle_tip_sync_watch() -> None:
    ...
    import json
    idle = json.loads((ROOT / "portable" / "BATCH378_IDLE.json").read_text(encoding="utf-8"))
    assert idle.get("batch") == "378"
    assert idle.get("lemma_closed") is False
    assert idle.get("flipped_anything") is False
    assert idle.get("tip_match") is True
    assert idle.get("action") == "idle_no_commit"
    assert _living_tip(str(idle.get("hardening_tip") or ""))
    unfreeze = json.loads((ROOT / "portable" / "BATCH378_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
    assert unfreeze.get("to_batch") == "378"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    ...
    wake = (ROOT / "scripts" / "post_batch322_wake_comments.py").read_text(encoding="utf-8")
    _wake_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', wake)]
    assert _wake_rets and max(_wake_rets) >= 378
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
...
def test_batch378_tip_or_eng_idle() -> None:
    ...
    import json
    import re
    tiny = json.loads((ROOT / "portable" / "BATCH378_TIP_ENG_IDLE.json").read_text(encoding="utf-8"))
    ...
    assert living.get("script_stale") == 0
    assert living.get("need_upload") == 0
    brief = json.loads((ROOT / "portable" / "BATCH378_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("action") == "idle_no_commit"
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_or_eng_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    hunt = json.loads((ROOT / "portable" / "BATCH378_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is False
    evidence = json.loads((ROOT / "portable" / "BATCH378_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("inv_lag_beyond_parent") is False
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 378)
...
def test_batch378_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH378_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "378"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    assert int(watch.get("open_prizes") or 0) >= 3
    assert _living_tip(str(watch.get("hardening_tip") or ""))
    delta = watch.get("delta_vs_batch377") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0
    assert not (ROOT / "portable" / "BATCH378_RESEARCH_STACK_AUDIT.json").exists()

    evidence = json.loads(
        (ROOT / "portable" / "BATCH378_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "378"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("goal") == "OPEN"
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 378 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 378 research-audit-watch)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
    assert "Batch 378" in log_md and "research_stack_audit_watch" in log_md
...
def test_batch379_idle_tip_sync_watch() -> None:
    ...
    import json
    idle = json.loads((ROOT / "portable" / "BATCH379_IDLE.json").read_text(encoding="utf-8"))
    assert idle.get("batch") == "379"
    assert idle.get("lemma_closed") is False
    assert idle.get("flipped_anything") is False
    assert idle.get("tip_match") is True
    assert idle.get("action") == "idle_no_commit"
    assert _living_tip(str(idle.get("hardening_tip") or ""))
    living = json.loads((ROOT / "portable" / "BATCH379_LIVING_REPUBLISH_BRIEF.json").read_text(encoding="utf-8"))
    assert living.get("action") == "living_script_stale_force_republish"
    assert living.get("living_tag") == "batch241-path-c-bundle"
    assert living.get("lemma_closed") is False
    unfreeze = json.loads((ROOT / "portable" / "BATCH379_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
    assert unfreeze.get("to_batch") == "379"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 379)
    inv_py = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', inv_py)]
    assert _inv_rets and max(_inv_rets) >= 379
    wake = (ROOT / "scripts" / "post_batch322_wake_comments.py").read_text(encoding="utf-8")
    _wake_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', wake)]
    assert _wake_rets and max(_wake_rets) >= 379
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(verify.get("refresh_batch") or 0) >= 379
    assert verify.get("lemma_closed") is False
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
...
def test_batch379_tip_sync_watch_idle_unfreeze() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH379_TIP_SYNC_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "379"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH379_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False

    brief = json.loads(
        (ROOT / "portable" / "BATCH379_BRIEF.json").read_text(encoding="utf-8")
    )
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_sync_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    ...
    unfreeze = json.loads(
        (ROOT / "portable" / "BATCH379_UNFREEZE_BRIEF.json").read_text(encoding="utf-8")
    )
    assert unfreeze.get("to_batch") == "379"
    assert unfreeze.get("lemma_closed") is False

    pin = json.loads(
        (ROOT / "portable" / "BATCH379_TIP_SYNC_INV_TIP_PIN_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert pin.get("action") == "inventory_preserve_durable_tip_pin"
...
def test_batch379_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH379_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "379"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    assert int(watch.get("open_prizes") or 0) >= 3
    assert _living_tip(str(watch.get("hardening_tip") or ""))
    delta = watch.get("delta_vs_batch378") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0
    assert not (ROOT / "portable" / "BATCH379_RESEARCH_STACK_AUDIT.json").exists()

    evidence = json.loads(
        (ROOT / "portable" / "BATCH379_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "379"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("goal") == "OPEN"
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 379 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 379 research-audit-watch)" in owner
    log_md = (ROOT / "docs" / "AUTONOMOUS_48H_LOG.md").read_text(encoding="utf-8")
...
def test_batch379_tip_or_eng_living_tgz() -> None:
    ...
    tiny = json.loads((ROOT / "portable" / "BATCH379_TIP_ENG_IDLE.json").read_text(encoding="utf-8"))
    assert tiny.get("aligned") is True
    assert tiny.get("action") == "living_republish"
    assert tiny.get("inventable_promoted") is False
    assert tiny.get("scientific_effect") == "NONE"
    assert _living_tip(str(tiny.get("hardening_tip") or ""))
    assert _living_tip(str(tiny.get("hardening_tip") or ""))
    ...
    assert living.get("script_stale") == 0
    assert living.get("need_upload") == 0
    brief = json.loads((ROOT / "portable" / "BATCH379_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("action") == "living_republish"
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_or_eng_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    hunt = json.loads((ROOT / "portable" / "BATCH379_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    assert hunt.get("defect") == "living_tgz_newer"
    evidence = json.loads((ROOT / "portable" / "BATCH379_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "living_republish"
    assert evidence.get("inv_lag_beyond_parent") is False
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 379)
    inv_py = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
...
def test_batch380_idle_tip_sync_watch() -> None:
    ...
    import json
    idle = json.loads((ROOT / "portable" / "BATCH380_IDLE.json").read_text(encoding="utf-8"))
    assert idle.get("batch") == "380"
    assert idle.get("lemma_closed") is False
    assert idle.get("flipped_anything") is False
    assert idle.get("tip_match") is True
    assert idle.get("action") == "idle_no_commit"
    assert _living_tip(str(idle.get("hardening_tip") or ""))
    living = json.loads((ROOT / "portable" / "BATCH380_LIVING_REPUBLISH_BRIEF.json").read_text(encoding="utf-8"))
    assert living.get("action") == "living_tgz_content_delta_republish"
    assert living.get("living_tag") == "batch241-path-c-bundle"
    assert living.get("lemma_closed") is False
    unfreeze = json.loads((ROOT / "portable" / "BATCH380_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
    assert unfreeze.get("to_batch") == "380"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 380)
    inv_py = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', inv_py)]
    assert _inv_rets and max(_inv_rets) >= 380
    wake = (ROOT / "scripts" / "post_batch322_wake_comments.py").read_text(encoding="utf-8")
    _wake_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', wake)]
    assert _wake_rets and max(_wake_rets) >= 380
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
    assert int(verify.get("refresh_batch") or 0) >= 380
    assert verify.get("lemma_closed") is False
    unblock = (ROOT / "scripts" / "print_owner_unblock.sh").read_text(encoding="utf-8")
...
def test_batch380_research_stack_audit_watch() -> None:
    ...
    watch = json.loads(
        (ROOT / "portable" / "BATCH380_RESEARCH_AUDIT_WATCH.json").read_text(
            encoding="utf-8"
        )
    )
    assert watch.get("batch") == "380"
    assert watch.get("lemma_closed") is False
    assert watch.get("flipped_anything") is False
    assert watch.get("tip_match") is True
    assert watch.get("action") == "research_stack_audit_watch"
    ...
    assert int(watch.get("open_prizes") or 0) >= 3
    assert _living_tip(str(watch.get("hardening_tip") or ""))
    delta = watch.get("delta_vs_batch379") or {}
    assert int(delta.get("open_premises") or 0) == 0
    assert int(delta.get("open_lemmas") or 0) == 0
    assert int(delta.get("open_prizes") or 0) == 0
    assert not (ROOT / "portable" / "BATCH380_RESEARCH_STACK_AUDIT.json").exists()

    evidence = json.loads(
        (ROOT / "portable" / "BATCH380_RESEARCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("batch") == "380"
    assert evidence.get("lemma_closed") is False
    assert evidence.get("flipped_anything") is False
    assert evidence.get("action") == "research_stack_audit_watch"
    assert evidence.get("goal") == "OPEN"
    ...
    assert "research_stack_audit_watch" in unblock
    land = (ROOT / "portable" / "LAND.md").read_text(encoding="utf-8")
    assert "STATUS (Batch 380 research-audit-watch)" in land
    owner = (ROOT / "docs" / "OWNER_ACTIONS_MAIN.md").read_text(encoding="utf-8")
...
def test_batch380_tip_or_eng_living_script_stale() -> None:
    ...
    tiny = json.loads((ROOT / "portable" / "BATCH380_TIP_ENG_IDLE.json").read_text(encoding="utf-8"))
    assert tiny.get("aligned") is True
    assert tiny.get("action") == "living_republish"
    assert tiny.get("inventable_promoted") is False
    assert tiny.get("scientific_effect") == "NONE"
    ...
    assert living.get("script_stale") == 0
    assert living.get("need_upload") == 0
    brief = json.loads((ROOT / "portable" / "BATCH380_TIP_ENG_BRIEF.json").read_text(encoding="utf-8"))
    assert brief.get("action") == "living_republish"
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_or_eng_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    hunt = json.loads((ROOT / "portable" / "BATCH380_TIP_ENG_HUNT.json").read_text(encoding="utf-8"))
    assert hunt.get("defect_shipped") is True
    assert hunt.get("defect") == "living_script_stale"
    living_b = json.loads((ROOT / "portable" / "BATCH380_TIP_ENG_LIVING_BRIEF.json").read_text(encoding="utf-8"))
    assert living_b.get("action") == "living_script_stale_force_republish"
    evidence = json.loads((ROOT / "portable" / "BATCH380_TIP_ENG_EVIDENCE.json").read_text(encoding="utf-8"))
    assert evidence.get("action") == "living_republish"
    assert evidence.get("inv_lag_beyond_parent") is False
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 380)
...
def test_batch383_tip_sync_watch_idle_parent_pin() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH383_TIP_SYNC_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "383"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    assert tiny.get("tip_match") is True
    assert tiny.get("aligned") is True
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH383_TIP_SYNC_EVIDENCE.json").read_text(encoding="utf-8")
    )
    assert evidence.get("action") == "idle_no_commit"
    assert evidence.get("lemma_closed") is False

    brief = json.loads(
        (ROOT / "portable" / "BATCH383_TIP_SYNC_BRIEF.json").read_text(encoding="utf-8")
    )
    _asg = str(brief.get("assignment") or "")
    assert _asg.startswith("tip_sync_watch_vs_BASE_TIP_")
    assert _living_tip(_asg.rsplit("_", 1)[-1])
    ...
    pin = json.loads(
        (ROOT / "portable" / "BATCH383_TIP_SYNC_INV_TIP_PIN_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert pin.get("action") == "inventory_preserve_durable_tip_pin"
...
def test_batch384_idle_tip_sync_watch() -> None:
    ...
    import json
    idle = json.loads((ROOT / "portable" / "BATCH384_IDLE.json").read_text(encoding="utf-8"))
    assert idle.get("batch") == "384"
    assert idle.get("lemma_closed") is False
    assert idle.get("flipped_anything") is False
    assert idle.get("tip_match") is True
    assert idle.get("action") == "idle_no_commit"
    assert _living_tip(str(idle.get("hardening_tip") or ""))
    living = json.loads((ROOT / "portable" / "BATCH384_LIVING_REPUBLISH_BRIEF.json").read_text(encoding="utf-8"))
    assert living.get("action") == "living_script_stale_force_republish"
    unfreeze = json.loads((ROOT / "portable" / "BATCH384_UNFREEZE_BRIEF.json").read_text(encoding="utf-8"))
    assert unfreeze.get("to_batch") == "384"
    refresh = (ROOT / "scripts" / "refresh_path_c_bundle.sh").read_text(encoding="utf-8")
    _assert_refresh_batch_tag_default_at_least(refresh, 384)
    inv_py = (ROOT / "scripts" / "refresh_ai_agent_access_inventory.py").read_text(encoding="utf-8")
    _inv_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', inv_py)]
    assert _inv_rets and max(_inv_rets) >= 384
    wake = (ROOT / "scripts" / "post_batch322_wake_comments.py").read_text(encoding="utf-8")
    _wake_rets = [int(x) for x in __import__("re").findall(r'return "(\d+)"', wake)]
    assert _wake_rets and max(_wake_rets) >= 384
    verify = json.loads((ROOT / "portable" / "path-c-applied-bundle" / "VERIFY.json").read_text(encoding="utf-8"))
...
def test_batch385_tip_sync_watch_idle_parent_pin() -> None:
    ...
    tiny = json.loads(
        (ROOT / "portable" / "BATCH385_TIP_SYNC_IDLE.json").read_text(encoding="utf-8")
    )
    assert tiny.get("batch") == "385"
    assert tiny.get("lemma_closed") is False
    assert tiny.get("flipped_anything") is False
    ...
    evidence = json.loads(
        (ROOT / "portable" / "BATCH385_TIP_SYNC_WATCH_EVIDENCE.json").read_text(
            encoding="utf-8"
        )
    )
    assert evidence.get("action") == "idle_no_commit"
    ...
    brief = json.loads(
        (ROOT / "portable" / "BATCH385_TIP_SYNC_WATCH_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    _asg = str(brief.get("assignment") or "")
    ...
    pin = json.loads(
        (ROOT / "portable" / "BATCH385_TIP_SYNC_INV_TIP_PIN_BRIEF.json").read_text(
            encoding="utf-8"
        )
    )
    assert pin.get("action") == "inventory_preserve_durable_tip_pin"
