...
3. **Path C after Path A:** rebase hardening (`chatgpt/drive-github-hardening-20260919`) onto the new `main`, then apply portable `apply_all` **0001–0004 + 0008–0015** (engineering hygiene only). Prefer `../../scripts/owner_land_path_c.sh` with write creds, or `../patches/apply_all.sh` on a writable checkout.
...
```bash
# From trial: remote ALIGNED + optional Path C apply/assert
./portable/pr2-landing/VERIFY_AFTER_MERGE.sh
# alignment only: SKIP_PATH_C=1 ./portable/pr2-landing/VERIFY_AFTER_MERGE.sh

# On the research tip checkout:
...
`VERIFY_AFTER_MERGE.sh` asserts `lemma_closed=false` after applying 0001–0004 + 0008–0015 when
