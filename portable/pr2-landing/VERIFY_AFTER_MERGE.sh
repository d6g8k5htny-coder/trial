...
# 0001–0008, run math_status_check + focused pytest, and assert
...
Usage: VERIFY_AFTER_MERGE.sh [--apply-path-c | --skip-path-c]
  ...
  After remote tip is ALIGNED:
    ...
    clone/use MAIN_CHECKOUT, apply_all 0001–0008, math_status + focused
...
if [[ "$SKIP_PATH_C" == "1" ]]; then
  ...
  echo "Next after PR #2: rebase hardening onto new main, then apply_all 0001–0008:"
