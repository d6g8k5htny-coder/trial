...
**Batch 244 (living tip / pack sync):** hardening tip **`02cfbfd`** (== BASE_TIP; [PR #71](https://github.com/d6g8k5htny-coder/main/pull/71) 0019 attestations RW @ `542e6ec`). Living release **`batch241-path-c-bundle`**. Patches **0001–0004+0008–0019** are **already on tip** — `owner_land_path_c.sh --from-bundle` dry-run exits 0 with `already_applied_on_tip=true` (`.bundle` ff-only may diverge; apply_all `--check` is the authority). Do **not** re-land Path C onto `02cfbfd`. `when_writable_land` → `idle_path_c_done` (0018+0019 markers true). `lemma_closed=false`. Scientific effect: **NONE**.
...
On tip **`02cfbfd`** expect dry-run / land to report **`already_applied_on_tip=true`** (no new commits). Prior release tags (`batch239` / `batch218` / `batch207` / …) are historical only — do not download them for new lands.
...
On a clone that already has BASE_TIP `02cfbfd` (shallow OK — Batch 170 E2E verified `--depth 1` and `--depth 80`):
...
git checkout -B cursor/portable-engineering-patches 02cfbfdf3ddc3120ddde6b67b0ff927115de6c4c
...
**Living tip note:** at `02cfbfd` the merge may refuse ff-only (historical applied range diverged); that is expected — patches are already on tip. Prefer `apply_all.sh --check` / `owner_land_path_c.sh --from-bundle --dry-run`.
...
```bash
git fetch origin chatgpt/drive-github-hardening-20260919 && git checkout -B cursor/portable-engineering-patches 02cfbfdf3ddc3120ddde6b67b0ff927115de6c4c && git am /path/to/path-c-on-hardening.patch && git push -u origin HEAD
```
