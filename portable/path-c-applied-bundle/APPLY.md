...
**Batch 244 (living tip / pack sync):** hardening tip **`fcad723`** (== BASE_TIP; [PR #71](https://github.com/d6g8k5htny-coder/main/pull/71) 0019 attestations RW @ `542e6ec`). Living release **`batch241-path-c-bundle`**. Patches **0001–0004+0008–0019** are **already on tip** — `owner_land_path_c.sh --from-bundle` dry-run exits 0 with `already_applied_on_tip=true` (`.bundle` ff-only may diverge; apply_all `--check` is the authority). Do **not** re-land Path C onto `fcad723`. `when_writable_land` → `idle_path_c_done` (0018+0019 markers true). `lemma_closed=false`. Scientific effect: **NONE**.
...
On tip **`fcad723`** expect dry-run / land to report **`already_applied_on_tip=true`** (no new commits). Prior release tags (`batch239` / `batch218` / `batch207` / …) are historical only — do not download them for new lands.
...
On a clone that already has BASE_TIP `fcad723` (shallow OK — Batch 170 E2E verified `--depth 1` and `--depth 80`):
...
git checkout -B cursor/portable-engineering-patches fcad72366743f5244eb26a0164e106419de78137
...
**Living tip note:** at `fcad723` the merge may refuse ff-only (historical applied range diverged); that is expected — patches are already on tip. Prefer `apply_all.sh --check` / `owner_land_path_c.sh --from-bundle --dry-run`.
...
```bash
git fetch origin chatgpt/drive-github-hardening-20260919 && git checkout -B cursor/portable-engineering-patches fcad72366743f5244eb26a0164e106419de78137 && git am /path/to/path-c-on-hardening.patch && git push -u origin HEAD
```
