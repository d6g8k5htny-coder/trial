#!/usr/bin/env bash
# Owner Path C land: apply portable engineering patches (0001–0004 + 0008–0012) onto the
...
Usage: owner_land_path_c.sh [--direct-push] [--help]
  ...
  (default)     Clone tip, apply_all 0001–0004 + 0008–0012, push branch, open PR.
...
if grep -q '__pycache__' tools/carriers_verify.py 2>/dev/null \
  ...
  echo "--- apply_all 0001–0004 + 0008–0012 ---"
  if ! "$APPLY_ALL"; then
...
if git diff --quiet && git diff --cached --quiet; then
  ...
  git add -A
  git commit -m "fix: portable engineering patches 0001-0004+0008-0012 (Path C)
...
TITLE="fix: portable engineering patches 0001-0004+0008-0012 (Path C)"
BODY="$(cat <<EOF
Path C via \`scripts/owner_land_path_c.sh\`.

Applies trial \`portable/patches/apply_all.sh\` (**0001–0004 + 0008–0012**) onto \`${BASE_REF}\`.
