...
#   2. Download latest Path C release on trial (batch179-path-c-bundle or newer):
...
Usage: owner_land_path_c.sh [--dry-run] [--from-bundle] [--direct-push] [--help]
  ...
  --from-bundle Prefer portable/path-c-applied-bundle/path-c-on-hardening.bundle
                ...
                (batch179-path-c-bundle or newer; prior batch169-path-c-bundle / batch142-path-c-bundle / batch142+ had patch-only).
...
if [[ "$FROM_BUNDLE" -eq 1 ]]; then
  if [[ -f "$BUNDLE_GIT" ]]; then
    ...
    die "missing $BUNDLE_GIT and $BUNDLE_PATCH (need path-c-applied-bundle from batch179-path-c-bundle or newer)"
  fi
fi
