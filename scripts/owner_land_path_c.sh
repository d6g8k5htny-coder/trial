#!/usr/bin/env bash
# Owner Path C land: apply portable engineering patches (0001–0008) onto the
...
Usage: owner_land_path_c.sh [--direct-push] [--help]
  ...
  (default)     Clone tip, apply_all 0001–0008, push branch, open PR.
...
if git diff --quiet && git diff --cached --quiet; then
  ...
  git add -A
  git commit -m "fix: portable engineering patches 0001-0008 (Path C)
...
TITLE="fix: portable engineering patches 0001-0008 (Path C)"
BODY="$(cat <<EOF
Path C via \`scripts/owner_land_path_c.sh\`.

Applies trial \`portable/patches/apply_all.sh\` (**0001–0008**) onto \`${BASE_REF}\`.
