...
echo "  release: batch169-path-c-bundle (.bundle + .patch + oneshot; PATH_C_RELEASE_TAG override ok)"
echo "Tip refresh helper (Batch 173): $ROOT/scripts/refresh_path_c_bundle.sh  # tip fetch → BASE_TIP → apply_all → .patch+.bundle → VERIFY.json"
echo "  dry-run: $ROOT/scripts/refresh_path_c_bundle.sh --dry-run  # exit 0 tip-stable / 1 TIP_DRIFT"
...
echo "  snapshot: portable/ALIGNED_DRIFT_SNAPSHOT.json  |  release: batch169-path-c-bundle"
echo "Path C ONE-SHOT (owner laptop): gh release download batch169-path-c-bundle -R d6g8k5htny-coder/trial -p 'trial-portable-main-fixes.tgz' -p 'path-c-on-hardening.bundle'"
