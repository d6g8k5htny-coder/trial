...
echo "  release: batch218-path-c-bundle (.bundle + .patch + oneshot+refresh+PATH_C_STATUS+watch/readme link-only; PATH_C_RELEASE_TAG override ok; prior batch199-path-c-bundle / batch180-path-c-bundle / batch179-path-c-bundle)"
...
echo "  snapshot: portable/ALIGNED_DRIFT_SNAPSHOT.json  |  release: batch218-path-c-bundle"
echo "Path C ONE-SHOT (owner laptop): gh release download batch218-path-c-bundle -R d6g8k5htny-coder/trial -p 'trial-portable-main-fixes.tgz' -p 'path-c-on-hardening.bundle'"
