...
echo "=== Batch 73 — PERMANENT window; ALIGNED @ 1c6e74b (PR #41); Path C BASE_TIP ${BASE_TIP_SHORT}; CI land-workflows-dry-run (no MAIN_PUSH_TOKEN); unrestricted/auto-approve; HOLD VOID ==="
...
echo "  snapshot: portable/ALIGNED_DRIFT_SNAPSHOT.json  |  threats: portable/BATCH73_OPEN_PR_THREATS.json"
...
echo "Batch 73: BASE_TIP ${BASE_TIP_SHORT} unchanged; write DENIED; no 0017; CI validates land-option-b + land-path-c dry-run without MAIN_PUSH_TOKEN."
echo "pack_portable.sh auto-globs RESTORE_PLAN_* + BATCH*_TOKEN_SEARCH + PATH_C_REBASE_* + BATCH*_RESEARCH_STACK_AUDIT + ALIGNED_DRIFT_SNAPSHOT + validate_land_workflows (Batch 64+/67+/68+/70+/72+/73)."
